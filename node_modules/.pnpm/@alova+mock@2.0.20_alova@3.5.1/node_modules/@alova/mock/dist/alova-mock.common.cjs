...
enable = shared.trueValue, delay = 2000, httpAdapter, mockRequestLogger = consoleRequestInfo, mock, onMockResponse = defaultMockResponse, onMockError = defaultMockError, matchMode = 'pathname' } = { mock: {} }) {
    ...
    const resolvedMockRequestLogger = mockRequestLogger === shared.trueValue ? consoleRequestInfo : mockRequestLogger;
    return (elements, method) => {
        ...
        let query = shared.isString(paramsConfig) ? parseQueryString(paramsConfig) : paramsConfig || {};
        ...
        if (mockDataRaw === shared.undefinedValue) {
            if (httpAdapter) {
                shared.isFn(resolvedMockRequestLogger) &&
        ...
        let isOnMockResponseError = shared.falseValue;
        resonpsePromise = resonpsePromise
            ...
            isOnMockResponseError = shared.trueValue;
            return Promise.all([
            ...
            return shared.promiseReject(onMockError(error, method));
        });
