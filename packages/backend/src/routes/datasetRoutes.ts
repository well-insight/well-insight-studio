...
async function projectExists(projectId: string): Promise<boolean> {
  return Boolean(await queryOne('SELECT 1 FROM projects WHERE id = ?', [projectId]));
}

async function wouldCreateFolderCycle(folderId: string, newParentId: string): Promise<boolean> {
  let cur = await DatasetFolderModel.findById(newParentId);
  while (cur) {
    if (cur.id === folderId) return true;
    cur = cur.parent_id ? await DatasetFolderModel.findById(cur.parent_id) : undefined;
  }
  return false;
}
...
function serializeDataset(dataset: Awaited<ReturnType<typeof DatasetEntityModel.findById>>) {
  ...
  try {
    form_schema = dataset.form_schema
      ? (JSON.parse(dataset.form_schema) as Record<string, unknown>)
      : null;
  } catch {
    form_schema = null;
  }
...
function normalizeRowDateTimes(
  ...
  const dateTimeFieldIds = new Set(
    fields.filter((field) => field.field_type === 'datetime').map((field) => field.id),
  );
  return Object.fromEntries(
    Object.entries(values).map(([key, value]) => [
      key,
      value !== null && typeof value === 'string' && dateTimeFieldIds.has(key)
  ...
  try {
    ...
    if (parent_id != null) {
      ...
      if (p.owner_id !== req.userId) {
        ...
        if (u?.role !== 'admin')
          return res.status(403).json({ success: false, error: '无权在该目录下创建' });
      }
...
router.put('/folders/:folderId', async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      !(await projectExists(body.project_id))
    ) {
      return res.status(400).json({ success: false, error: '项目不存在' });
    }
    ...
    res.json({
      success: true,
      data: await DatasetFolderModel.findById(folderId),
      message: '目录已更新',
    });
  } catch (error) {
    if (error instanceof z.ZodError) {
...
router.post('/:datasetId/rows/batch', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, data: { count: body.rows.length }, message: '数据已导入' });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ success: false, error: '数据验证失败', details: error.errors });
    }
...
router.post('/:datasetId/rows', async (req: Request, res: Response) => {
  try {
    ...
    const id = await DatasetRowModel.create(
      datasetId,
      JSON.stringify(normalizeRowDateTimes(fields, body.values)),
      body.sort_order,
    );
...
router.put('/:datasetId/rows/:rowId', async (req: Request, res: Response) => {
  try {
    ...
    if (err) return res.status(400).json({ success: false, error: err });
    const ok = await DatasetRowModel.update(
      rowId,
      datasetId,
      JSON.stringify(normalizeRowDateTimes(fields, merged)),
      body.sort_order,
    );
