...
async function assertFormPage(pageId: string, userId: string) {
  ...
  if (!page || page.created_by !== userId) return null;
  if (page.type !== 'form') throw new Error('NOT_FORM_PAGE');
  return page;
}
...
router.post('/folders', async (req: Request, res: Response) => {
  try {
    ...
    if (body.parent_id != null) {
      ...
      if (!parent) return res.status(400).json({ success: false, error: '父目录不存在或无权限' });
    }
...
router.put('/folders/:folderId', async (req: Request, res: Response) => {
  try {
    ...
    if (!existing) return res.status(404).json({ success: false, error: '目录不存在' });
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (body.folder_id != null) {
      ...
      if (!folder) return res.status(400).json({ success: false, error: '所属目录不存在或无权限' });
    }
...
router.get('/:id/records', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      success: true,
      data: { items: result.items, total: result.total, page: queryPage, pageSize },
    });
  } catch (error) {
    if (error instanceof z.ZodError)
      return res.status(400).json({ success: false, error: '参数验证失败', details: error.errors });
...
router.post('/:id/records', async (req: Request, res: Response) => {
  try {
    ...
    const record = await FormRecordModel.create({
      page_id: pageId,
      values: body.values,
      created_by: req.userId!,
      sort_order: body.sort_order,
