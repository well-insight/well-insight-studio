...
function parseRow(row: Page): PagePublic {
  return {
    ...row,
    folder_id: row.folder_id ?? null,
    dsl: json(row.dsl || '{}'),
    dataset_bindings: row.dataset_bindings ? json(row.dataset_bindings) : null,
...
export class PageModel {
  ...
  static async findById(id: string): Promise<PagePublic | undefined> {
    const row = await queryOne<Page>('SELECT * FROM pages WHERE id = ?', [id]);
  ...
  static async findAll(options?: {
    ...
    page?: number;
    pageSize?: number;
  }): Promise<{ items: PagePublic[]; total: number }> {
    ...
    if (options?.folder_id !== undefined) {
      if (options.folder_id === null) conditions.push('folder_id IS NULL');
      else {
    ...
    const count = await queryOne<{ count: number | string }>(
      `SELECT COUNT(*) AS count FROM pages ${where}`,
      params,
    );
    ...
    const rows = await query<Page[]>(
      `SELECT * FROM pages ${where} ORDER BY updated_at DESC LIMIT ${pageSize} OFFSET ${offset}`,
      params,
    );
  ...
  ): Promise<PagePublic | undefined> {
    const existing = await queryOne<Page>('SELECT * FROM pages WHERE id = ?', [id]);
    if (!existing) return undefined;
    await execute(
      'UPDATE pages SET folder_id = ?, name = ?, type = ?, dsl = ?, dataset_bindings = ?, preview_url = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [
        data.folder_id !== undefined ? data.folder_id : (existing.folder_id ?? null),
        data.name ?? existing.name,
        data.type ?? existing.type,
        data.dsl !== undefined ? JSON.stringify(data.dsl) : existing.dsl,
        data.dataset_bindings !== undefined
          ? JSON.stringify(data.dataset_bindings)
          : (existing.dataset_bindings ?? null),
  ...
  static async delete(id: string): Promise<boolean> {
    return (await execute('DELETE FROM pages WHERE id = ?', [id])).affectedRows > 0;
  }
}
