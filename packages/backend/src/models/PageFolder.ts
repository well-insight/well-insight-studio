...
export class PageFolderModel {
 ...
 static async create(data: { parent_id?: string | null; name: string; description?: string | null; owner_id: string; sort_order?: number; }): Promise<PageFolder> { const id = generateSnowflakeId(); await execute("INSERT INTO page_folders (id, parent_id, name, description, owner_id, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)", [id, data.parent_id ?? null, data.name, data.description ?? null, data.owner_id, data.sort_order ?? 0]); return (await this.findById(id))!; }
 static async update(id: string, data: Partial<Pick<PageFolder, "parent_id" | "name" | "description" | "sort_order">>): Promise<PageFolder | undefined> { const row = await this.findById(id); if (!row) return undefined; await execute("UPDATE page_folders SET parent_id = ?, name = ?, description = ?, sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [data.parent_id !== undefined ? data.parent_id : row.parent_id, data.name ?? row.name, data.description !== undefined ? data.description : row.description, data.sort_order ?? row.sort_order, id]); return this.findById(id); }
