...
async function bootstrap(): Promise<void> {
  ...
  const server = app.listen(PORT, () => {
    ...
    console.log('[DATABASE] MySQL 数据库已就绪');
  });
  ...
  const shutdown = (signal: string) => {
    ...
    setTimeout(() => void closeDatabase().finally(() => process.exit(1)), 8000).unref();
  };
...
process.on('uncaughtException', (err) => console.error('[SERVER] 未捕获异常，服务继续运行:', err));
