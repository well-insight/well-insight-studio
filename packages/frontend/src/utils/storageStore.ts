/**
 * @description 对 sessionStorage、localStorage 的封装（非 Pinia，仅为存储工具）
 */

export function useStorageStore(param: string, pre?: string) {
  ...
  function set(name: string, value = '', time = 0) {
    ...
    obj[`${prefix}startTime`] = Date.now() // 记录何时将值存入缓存，毫秒级
  ...
  function get(name: string) {
    ...
    if (item) {
      ...
      if (
        item[`${prefix}startTime`] > 0
        && !Number.isNaN(item[`${prefix}startTime`])
        && !Number.isNaN(item[`${prefix}time`])
      ) {
