  ...
  render: ({ styles, block, props }) => {
    ...
    const inputProps = computed(() => {
      ...
      if (props.type === 'textarea') {
        if (props.rows) base.rows = props.rows
        if (props.autosize) base.autosize = props.autosize
        if (props.resize) base.resize = props.resize
      }
    ...
    return () => (
      <div style={{ width: '100%', height: '100%', ...styles }}>
        <ElFormItem
          ...
          rules={props.rules}
        >
          <ElInput ref={el => registerRef(el, block._vid)} v-model={props.modelValue} {...inputProps.value} />
        </ElFormItem>
      </div>
    )
  },
