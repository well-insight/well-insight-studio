...
export default {
  ...
  methods: {
    ...
    handleLineChange(arr, vertical) {
      const newLines = vertical ? { h: this.horLineArr, v: [...arr] } : { h: [...arr], v: this.verLineArr }
...
<template>
  <div id="mb-ruler" class="style-ruler mb-ruler">
    ...
    <RulerWrapper
      ...
      @on-line-change="handleLineChange"
    />
    <!-- 竖直方向 -->
    <RulerWrapper
      ref="vRuler"
      ...
      @on-line-change="handleLineChange"
    />
    <a class="corner" :class="cornerActiveClass" :style="cornerStyle" @click="onCornerClick" />
  </div>
</template>
