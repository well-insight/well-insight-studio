...
export default {
  ...
  mounted() {
    ...
    addEvent(this.getEventScopeElement(), 'mousedown', this.deselect)
    addEvent(this.getEventScopeElement(), 'touchend touchcancel', this.deselect)
    //  窗口变化时，检查容器大小
    this.checkParentSize()
    addEvent(window, 'resize', this.checkParentSize)
  },
  beforeUnmount() {
    removeEvent(this.getEventScopeElement(), 'mousedown', this.deselect)
    removeEvent(document.documentElement, 'touchstart', this.handleUp)
    removeEvent(document.documentElement, 'mousemove', this.move)
    removeEvent(document.documentElement, 'touchmove', this.move)
    removeEvent(document.documentElement, 'mouseup', this.handleUp)
    removeEvent(this.getEventScopeElement(), 'touchend touchcancel', this.deselect)
    removeEvent(window, 'resize', this.checkParentSize)
  },
  methods: {
    getEventScopeElement() {
      if (this.eventScope) {
        const element = document.querySelector(this.eventScope)
        if (element) {
          return element
        }
      } else {
    ...
    getParentSize() {
      if (typeof this.parent === 'string') {
        ...
        if (!(parentNode instanceof HTMLElement)) {
          throw new TypeError(`The selector ${this.parent} does not match any element`)
        }
    ...
    async handleDrag(e) {
      ...
      let right = restrictToBounds(mouseClickPosition.right + deltaX, bounds.minRight, bounds.maxRight)
      let bottom = restrictToBounds(mouseClickPosition.bottom + deltaY, bounds.minBottom, bounds.maxBottom)
      // 精度控制
      if (typeof this.precision === 'number') {
        left = round(left, this.precision)
        top = round(top, this.precision)
        right = round(right, this.precision)
        bottom = round(bottom, this.precision)
      }
      this.left = left
      this.top = top
      this.right = right
    ...
    handleResize(e) {
      ...
      // 精度控制
      if (typeof this.precision === 'number') {
        newW = round(newW, this.precision)
        newH = round(newH, this.precision)
      }
          ...
          if (
            ...
            if ((rotate - this.rotate) % 90 === 0) {
              ...
              if (bln) {
                ...
                // 和容器贴边
                if (this.snapBorder) {
                  console.log(this.top, this.right, this.bottom, this.left, this.parentWidth, this.parentHeight)
