  ...
  computed: {
    handleStyle() {
      return (stick) => {
        ...
        if (stick === 'rot' && !this.rotatable)
          return { display: 'none' }
        if (stick !== 'rot' && !this.resizable)
          return { display: 'none' }
        ...
        if (stick !== 'rot') {
          ...
          const index = (mapStick2Index[stick] + deltaIndex) % 8
    ...
    // 控制柄显示与否
    actualHandles() {
      if (!this.resizable && !this.rotatable)
        return []
      return this.handles
    },
  ...
  methods: {
    ...
    handleResize(e) {
      ...
      if (!this.rotatable && this.parent) {
        mouseX = restrictToBounds(
          mouseX,
          this.parentX,
          this.parentX + this.parentWidth * scaleRatio,
        )
        mouseY = restrictToBounds(
          mouseY,
          this.parentY,
          this.parentY + this.parentHeight * scaleRatio,
        )
      }
      // 获取鼠标移动的坐标差
