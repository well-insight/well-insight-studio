...
Theme support is built into `@well-design/ui`. Components consume semantic CSS variables (`--wd-*`) and do not keep a second palette.

Importing `@well-design/ui/styles.css` already includes these variables. Theme JS APIs (`useTheme` and friends) come from the same package.
...
At the app level use `createWellDesign({ density: 'compact' })` or `<WdConfigProvider density="compact">`.
...
Theme switching is the visual layer. `WdConfigProvider` / `createWellDesign` own size, copy, overlay mount, and other behavioral defaults. Use both together; see [Configuration](/docs/config).
