...
- Light / dark themes, `useTheme` / `useDensity` / `useMotion`, plus `WdConfigProvider` / `createWellDesign` for global defaults.
- Build output is ESM + type declarations + `styles.css`. Inside the monorepo, `exports.development` points at source.
