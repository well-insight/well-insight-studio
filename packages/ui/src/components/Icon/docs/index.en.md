...
`WdIcon` only maintains **component-library system icons** (close, arrows, status, actions, and so on). For full business icon sets, use the default slot with [Lucide](https://lucide.dev) or another library so hundreds of SVGs are not bundled into `@well-design/ui`.

## Import

```ts
import { WdIcon, iconNames } from '@well-design/ui'
```
...
import { WdIcon, WdInput, WdToast, iconNames, type ToastMessage } from '@well-design/ui'
...
import { WdIcon, WdButton, WdIconField, WdInput } from '@well-design/ui'
</script>
