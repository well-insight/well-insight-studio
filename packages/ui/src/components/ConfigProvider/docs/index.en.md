...
Provide global defaults for the component tree via `WdConfigProvider` or `createWellDesign`. Local props take precedence over global config.
...
Precedence: **component props > `WdConfigProvider` > `createWellDesign()` > built-in defaults**.

## Theme and motion

Theme and motion APIs are also exported from `@well-design/ui` and can be used alongside ConfigProvider:
